...
export default function Rdapaciente({ auth, transmisiones, filters, configuraciones = [], globalMetrics = {}}) {
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(() => {
            router.get(
                route('ihce.transmision.rdaconsulta'), 
                { search, estado }, 
    ...
    const handleSort = (columnName) => {
        ...
        router.get(
            route('ihce.transmision.rdaconsultaexterna'), 
    ...
    const [globalKey, setGlobalKey] = useState(0); // Pequeño truco para resetear o colapsar todo dinámicamente

    const handleCopyJson = (textJson) => {
        navigator.clipboard.writeText(JSON.stringify(textJson, null, 2));
        setCopiedModal(true);
        setTimeout(() => setCopiedModal(false), 2000);
    };


const [collapseAll, setCollapseAll] = useState(false); // Sostiene si el árbol nace colapsado o no

const handleToggleAll = (shouldCollapse) => {
    setCollapseAll(shouldCollapse);
    setGlobalKey(prev => prev + 1); // Forzamos el re-render estructural del árbol interactivo
};


    return (
        <AuthenticatedLayout user={auth.user} header="Visor de Transmisiones e Interoperabilidad: RDA Consulta Externa">
            ...
            {/* 🚀 EL INTERRUPTOR DEL MODAL EXPANDIDO PREMIUM */}
            {expandedSection && (
                <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-4 sm:p-6 animate-in fade-in duration-200">
                    <div className="bg-[#070a12] border border-slate-800/80 rounded-2xl w-full max-w-5xl h-[85vh] flex flex-col p-5 sm:p-6 shadow-[0_0_50px_-12px_rgba(0,0,0,0.7)] border-t-indigo-500/20 relative overflow-hidden">
                        
                        {/* Luces de fondo sutiles estéticas tipo Cyberpunk */}
                        <div className="absolute top-0 right-1/4 w-96 h-24 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none" />
                        
                        {/* Cabecera del Modal */}
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4 mb-4 z-10">
                            <div className="flex items-center gap-2.5">
                                <span className={`w-2.5 h-2.5 rounded-full ${expandedSection.badge} shadow-[0_0_10px_2px_currentColor]`}></span>
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mt-0.5">
                                        Visualizador Clínico de Recursos Estructurados
                                    </span>
                                </div>
                            </div>

                            {/* Grupo de Acciones del Panel */}
                            <div className="flex flex-wrap items-center gap-2">
                                
                                {/* Botonera de Control de Árbol Dinámico */}
                                <div className="inline-flex p-0.5 bg-slate-900 border border-slate-800 rounded-lg shadow-inner">
                                    <button
                                        onClick={() => handleToggleAll(false)}
                                        className={`inline-flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-1.5 rounded-md transition-all cursor-pointer ${
                                            !collapseAll 
                                ...
                                {/* Botón de Copiado con Feedback de Color */}
                                <button
                                    onClick={() => handleCopyJson(expandedSection.data)}
                                    className={`inline-flex items-center gap-1.5 text-[10px] font-mono font-black uppercase tracking-wider border px-3 py-1.5 rounded-lg transition-all duration-200 cursor-pointer ${
                                        copiedModal 
                                ...
                                >
                                    {copiedModal ? (
                                        <>
                                            <HiCheck className="w-3.5 h-3.5 text-emerald-400" /> ¡Copiado!
                                        </>
                                    ) : (
                                        <>
                                            <HiClipboardCopy className="w-3.5 h-3.5 text-slate-400" /> Copiar Código
                                        </>
                                    )}
                                </button>
                                ...
                                <button 
                                    ...
                                    className="inline-flex items-center gap-1.5 text-[10px] font-mono font-black text-slate-400 hover:text-rose-400 uppercase tracking-wider bg-slate-900 hover:bg-rose-950/20 border border-slate-800 hover:border-rose-900/50 px-3 py-1.5 rounded-lg transition-all cursor-pointer"
                                >
                                    <HiX className="w-3.5 h-3.5" /> Cerrar Terminal
                                </button>
                            </div>
                        </div>
                        
                        {/* Contenedor del Árbol Interactivo (Diseño Dark Arcade Pure) */}
                        <div className="flex-1 overflow-auto bg-[#03050a] border border-slate-800/70 rounded-xl p-5 custom-scrollbar shadow-inner text-left relative group">
                            <div 
                                key={globalKey} 
                                className="p-1 select-text selection:bg-indigo-500/20 selection:text-indigo-200 transition-opacity duration-150 animate-in fade-in"
                            >
                                <JsonTreeView 
                                    data={expandedSection.data} 
                                    isLast={true} 
                                    isInitiallyCollapsed={collapseAll} 
                                />
                            </div>
                        </div>

                        {/* Footer Informativo del Nodo */}
                        <div className="mt-3.5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-[10px] text-slate-500 font-mono tracking-wide border-t border-slate-900 pt-3">
                            <div className="flex items-center gap-1.5">
                                <span className="text-indigo-400 font-bold">💡 Tip:</span> 
                                <span>Puedes interactuar con los nodos haciendo clic sobre las llaves primarias abiertas.</span>
                            </div>
                            <div className="bg-slate-900/80 px-2 py-0.5 rounded border border-slate-800/60 text-slate-400 font-semibold uppercase tracking-wider">
                                Estructura: {Array.isArray(expandedSection.data) ? 'Array (Collection)' : 'Object (Resource)'}
                            </div>
                        </div>
                    </div>
                </div>
